...
impl Renderable for Note {
	...
	if (win_h > self.time_ms) && (time < self.time_ms) {
	    ...
	    if self.x == 64 || self.x == 448 {
		rectangle(PINK, [self.x as f64+300., y,64.0,-32.0], c.transform, g);	
	    } else {
		rectangle(WHITE, [self.x as f64+300., y,64.0,-32.0], c.transform, g);
	    }
...
impl Renderable for LongNote {
	...
	if (self.typ == 128) && (win_h > self.time_ms) && (time < self.endtime) {
		...
		rectangle(WHITE, [self.x as f64+300., y_top, 64.0, y_height], c.transform, g);
	    }
	...
	if let Ok(lines) = read_lines(filename) {
		...
		if let Ok(s) = line {
			...
			match &t[1] {
			    ...
			    "HPDrainRate" => beatmap.hpDrainRate = t[3].to_string().parse().unwrap_or(0),
			    "CircleeSize" => beatmap.circleSize = t[3].to_string().parse().unwrap_or(0),
			    "OverallDifficulty" => beatmap.overallDifficulty = t[3].to_string().parse().unwrap_or(0),
			    "ApproachRate" => beatmap.approachRate = t[3].to_string().parse().unwrap_or(0),
			    "SliderTickRate" => beatmap.silderTickRate = t[3].to_string().parse().unwrap_or(0),
		  	    _ => {},	
			}
			if &t[3] == "" {
			    let spilt = s.split(",");
